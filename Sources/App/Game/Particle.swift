...
class Particle {
    ...
    static func explosion(position: Vector2, velocity: Vector2, 
        minVelocityAngle: Angle, maxVelocityAngle: Angle,
        color: Color, particleSize: Double, particleCount: Int, minDuration: Double, maxDuration: Double) -> [Particle] {
        var particles: [Particle] = []
        for _ in 0..<particleCount {
            let direction = Vector2(angle: .degrees(Double.random(in: minVelocityAngle.degrees...maxVelocityAngle.degrees)))
